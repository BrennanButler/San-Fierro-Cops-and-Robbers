...
{
	new rows, fields, hPass[250], temp[12];
	...
	if(rows) // Account was found, attempt to login player.
	{
		...
		// Load the data for the potential player
		// There's no point in discarding the data loaded into these variables as they are overwritten by a new player
		// and there is no security risk as players can not get a hold of server data
		cache_get_row(0, 3, PlayerInfo[playerid][Ppassword], 1, 250);
		cache_get_row(0, 4, temp); SetPlayerScore(playerid, strval(temp));
		cache_get_row(0, 5, temp); GivePlayerMoney(playerid, strval(temp));
		cache_get_row(0, 6, temp); PlayerInfo[playerid][Pbank] = strval(temp);
		cache_get_row(0, 7, temp); PlayerInfo[playerid][Pstat] = strval(temp);
		cache_get_row(0, 8, temp); PlayerInfo[playerid][Parmy] = strval(temp);
		cache_get_row(0, 9, temp); PlayerInfo[playerid][Pswat] = strval(temp);
		cache_get_row(0, 10, temp); PlayerInfo[playerid][Padmin] = strval(temp);
		cache_get_row(0, 11, temp); PlayerInfo[playerid][Pcop] = strval(temp);
		cache_get_row(0, 12, temp); PlayerInfo[playerid][Pscore] = strval(temp);
		cache_get_row(0, 13, temp); PlayerInfo[playerid][Pprison] = strval(temp);
		cache_get_row(0, 14, temp); PlayerInfo[playerid][Pvip] = strval(temp);
		cache_get_row(0, 15, temp); PlayerInfo[playerid][Parrest] = strval(temp);
		cache_get_row(0, 16, temp); PlayerInfo[playerid][Psurender] = strval(temp);
		cache_get_row(0, 17, temp); PlayerInfo[playerid][Prob] = strval(temp);
		cache_get_row(0, 18, temp); PlayerInfo[playerid][Prape] = strval(temp);
		cache_get_row(0, 19, temp); PlayerInfo[playerid][Pheal] = strval(temp);
		cache_get_row(0, 20, temp); PlayerInfo[playerid][Phitman] = strval(temp);
		cache_get_row(0, 21, temp); PlayerInfo[playerid][Psales] = strval(temp);
		cache_get_row(0, 22, temp); PlayerInfo[playerid][Pwep] = strval(temp);
		cache_get_row(0, 23, temp); PlayerInfo[playerid][Pdrug] = strval(temp);
		cache_get_row(0, 24, temp); PlayerInfo[playerid][Ptaze] = strval(temp);
		cache_get_row(0, 25, temp); PlayerInfo[playerid][Ptruck] = strval(temp);
		cache_get_row(0, 26, temp); format(PlayerInfo[playerid][STD], 100, temp);
		cache_get_row(0, 27, temp); PlayerInfo[playerid][PrisonTime] = strval(temp);
		cache_get_row(0, 28, temp); PlayerInfo[playerid][Mayor] = !!strval(temp);
		cache_get_row(0, 29, temp); PlayerInfo[playerid][PTERRORIST] = strval(temp);
		cache_get_row(0, 30, temp); PlayerInfo[playerid][Pfix] = strval(temp);
		cache_get_row(0, 31, temp); PlayerInfo[playerid][Pkidnapper] = strval(temp);
		cache_get_row(0, 32, temp); PlayerInfo[playerid][Ppedo] = strval(temp);
		cache_get_row(0, 33, temp); PlayerInfo[playerid][Ptaxi] = strval(temp);
		cache_get_row(0, 34, temp); PlayerInfo[playerid][Ppilot] = strval(temp);
		cache_get_row(0, 35, temp); PlayerInfo[playerid][Ppizza] = strval(temp);
		cache_get_row(0, 36, temp); PlayerInfo[playerid][Pfarmer] = strval(temp);
		cache_get_row(0, 37, temp); PlayerInfo[playerid][Pminer] = strval(temp);
